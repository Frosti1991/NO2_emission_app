...
def emission(request):
    def get_data():
        # format the dates
        date_from_str = (datetime.now()-timedelta(hours=24)
                         ).strftime("%Y-%m-%d")
        ...
        for i in range(24, 2, -1):
            ...
            obj, created = Emission.objects.get_or_create(time_date=json_time_save, defaults={
                                                          'station_id': station, 'emission': NO_value})
            obj.save()
...
def login_func(request):
    ...
    if request.method == 'POST':
        # gets the username of dicitionary from submitted login html
